...
$sql = "SELECT * FROM `aluno` WHERE id = $Matricula and Foto is not null";
...
echo "<header style='background:white; margin-top:-10px; padding:5px;'>
    <ul>
        <a href='../authenticated/home.php'> <li>
            <img src='..\imagens/jmtg__2_-removebg-preview.png' alt=''class='logo'> JMTG
        </li></a> 
        </li></a>
  ...
  <a href='./logout.php'><li>Sair</li></a>
    ...
    echo "<header style='background:#030253; margin-top:-10px; padding:5px; padding:5px;'>
      <ul>
          <a href='../authenticated/home.php'> <li>
...
$sql = "SELECT * FROM `aluno` WHERE id = $Matricula";
$result = $conn->query($sql);
    ...
    while($row = $result->fetch_assoc()) {
        echo "<form method='POST' action='' enctype='multipart/form-data'>
            ...
            <div class='container-inputs'>
                ...
                <input type='number' value='{$row['Telegone']}' name='celular'>
            </div>
...
if (isset($_POST['atualizar_cadastro'])) {
    ...
    if (isset($_FILES["Foto"]) && $_FILES["Foto"]["error"] === UPLOAD_ERR_OK) {
        ...
        $sql = "SELECT foto FROM aluno WHERE id=$Matricula";
        $result = $conn->query($sql);
        $row = $result->fetch_assoc();
        $foto_atual = $row["Foto"];

        // Define o novo nome da imagem no banco de dados
        $sql = "UPDATE cadastro SET foto ='$new_file_name', nome='$Nome', email='$Email', cpf='$CPF', Matricula='$Matricula', telefone='$Telefone', WHERE id=$Matricula";
        ...
        $sql = "UPDATE aluno SET nome='$Nome', Email='$email', cpf='$CPF', matricula='$Matricula', telefone='$Telefone',  WHERE id=Matricula";
        if ($conn->query($sql) === TRUE) {
            echo "Cadastro atualizado com sucesso!";
